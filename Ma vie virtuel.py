...
print("sys:Taper 4 pour ouvrir une sauvegarde d'une version précédente")
try:
    entré = int(input())
except ValueError:
...
try:
    ...
    #version = partie [0]
    #if version != "0.0.2":
        #print("Il à été détécté que votre sauvegarde provient d'une ancienne version! Veuillez la convertir en tapant 4 au prochain démarrage.")
    ...
    print("Cette erreur est survenue car votre sauvegarde est introuvable ou alors ne possède plus son nom d'origine, vérifiez qu'il possède un nom de type sauvegarde[chiffre].txt")
    print("Le jeu va s'arrêter")
    time.sleep(20)
    sys.exit()
...
if entré == 4:
    ...
    time.sleep(5)
    print("sys: Cette version à désactivé l'assistant de mise à jour de sauvegarde car les versions précédentes ne permettaient pas encore de sauvegarder une partie")
    print("Le jeu va s'arrêter")
    time.sleep(10)
    sys.exit()
    #try:
...
while True:
    if état_partie == "0":
        ...
        time.sleep(8)
        print("[PC]John: ATTENTION: Il s'agit du tutoriel de la version ALPHA 0.0.2, IL EST INCOMPLET")
        time.sleep(7)
